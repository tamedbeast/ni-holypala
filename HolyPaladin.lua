...
local abilities = {
    ["layOnHands"] = function()
        if enables["heal"] and ni.player.hp() <= values["layOnHandsThreshold"] and ni.spell.available(spells.layOnHands) and not ni.player.ismounted() and not UnitIsDeadOrGhost("player") and not ni.spell.gcd() then
    ...
    ["divineShield"] = function()
        if ni.player.hp() <= values["divineShieldThreshold"] and ni.spell.available(spells.divineShield) and not ni.player.ismounted() and not UnitIsDeadOrGhost("player") and not ni.spell.gcd() then
            ni.spell.stopcasting()
    ...
    ["handOfProtection"] = function()
        ...
        if allBelowThreshold and ni.spell.available(spells.handOfProtection) and not ni.player.ismounted() and not UnitIsDeadOrGhost("player") and not ni.spell.gcd() then
            ni.spell.stopcasting()
